...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  private async initializeDatabase() {
    ...
    const user = this.configService.get<string>('DB_USER', 'allergy_admin');
    const password = this.configService.get<string>('DB_PASSWORD', 'allergy_password');
    const database = this.configService.get<string>('DB_NAME', 'allergyfree_db');
    ...
    try {
      ...
      this.logger.warn(`PostgreSQL connection failed: ${(err as Error).message}`);
  ...
  private async loadSchemaAndSeed() {
    try {
      ...
      this.logger.error(`Error during SQLite schema/seeding: ${(err as Error).message}`);
    }
  }

  private translateToSqlite(sql: string): string {
    return sql
      ...
      // Remove PostgreSQL specific index keywords if necessary, but standard CREATE INDEX is fine.
      ;
  }

  private async executeMultiStatementSqlite(sql: string): Promise<void> {
  ...
  private async runSqliteQuery(sql: string, params: any[] = []): Promise<any> {
    return new Promise((resolve, reject) => {
  ...
  private async ensureAllergens() {
    try {
      ...
      for (const a of standardAllergens) {
        ...
        await this.query(
          this.isSqlite ? sql.replace('$1', '?').replace('$2', '?').replace('$3', '?') : sql,
          [a.name, a.display_name, a.severity]
        );
      }
      this.logger.log('Standard allergens verified and inserted.');
    } catch (err) {
      this.logger.error(`Error ensuring standard allergens: ${(err as Error).message}`);
    }
  }

  private async ensureAllergenAliasTable() {
    try {
      ...
      this.logger.error(`Error ensuring allergen_alias table: ${(err as Error).message}`);
    }
  }

  private async seedAllergenAliases() {
    try {
      const rows = await this.query<any>('SELECT COUNT(*) as count FROM allergen_alias');
      ...
      for (const item of aliases) {
        // Find allergen id by name
        const findSql = `SELECT id FROM allergens WHERE name = $1 LIMIT 1`;
        const allergens = await this.query<any>(
          this.isSqlite ? findSql.replace('$1', '?') : findSql,
          [item.name]
        );
        ...
        for (const kw of item.keywords) {
          ...
          `;
          await this.query(
            this.isSqlite ? insertSql.replace('$1', '?').replace('$2', '?') : insertSql,
            [allergenId, kw]
          );
        }
      }
      ...
      this.logger.error(`Error seeding allergen_alias: ${(err as Error).message}`);
    }
  }
